...
Stmt Parser::statement()
{
    ...
    if(match(Token_Type::LEFT_BRACE))
    {
        ...
        return Stmt::Block_Stmt(statements);
    }
...
Stmt Parser::forStatement()
{
    ...
    {
        vector<shared_ptr<Stmt>> blockList = {make_shared<Stmt>(body), make_shared<Stmt>(Stmt::Expression_Stmt(*increment))};
        body = Stmt::Block_Stmt(blockList);
    }
    
    if(!condition.has_value())
    {
        bool literalTrue = true;
        condition = Expr::Literal_Expr(literalTrue);
    }
    ...
    {
        vector<shared_ptr<Stmt>> blockList = {make_shared<Stmt>(*initializer), make_shared<Stmt>(body)};
        body = Stmt::Block_Stmt(blockList);
    }
...
Stmt Parser::ifStatement()
{
    ...
    return Stmt::If_Stmt(condition, thenBranch, elseBranch);
}
...
Stmt Parser::varDeclaration()
{
    ...
    consume(tokenType, message);
    return Stmt::Var_Stmt(name, initializer);
}
...
Stmt Parser::whileStatement()
{
    ...
    return Stmt::While_Stmt(condition, body);
}
...
Stmt Parser::expressionStatement()
{
    ...
    consume(tokenType, message);
    return Stmt::Expression_Stmt(expr);
}
...
Expr Parser::assignment()
{
    ...
    if(match(Token_Type::EQUAL))
    {
        ...
        if(typeid(expr) == typeid(Expr::Variable_Expr))
        {
            ...
            return Expr::Assign_Expr(name, value);
        }
...
Expr Parser::logicalOr()
{
    ...
    while(match(Token_Type::OR))
    {
        ...
        Expr right = equality();
        expr = Expr::Logical_Expr(expr, operatorToken, right);
    }
...
Expr Parser::equality()
{
    ...
    while(match(Token_Type::BANG_EQUAL, Token_Type::EQUAL_EQUAL))
    {
        ...
        expr = Expr::Binary_Expr(expr, operatorToken, right);
    }
...
Expr Parser::unary()
{
    if(match(Token_Type::BANG, Token_Type::MINUS))
    {
        Interpreter_Token operatorToken = previous();
        Expr right = unary();
        return Expr::Unary_Expr(operatorToken, right);
    }
    return call();
}
Expr Parser::finishCall(Expr& callee)
{
    ...
    return Expr::Call_Expr(callee, paren, arguments);
}
...
Expr Parser::primary()
{
    ...
    {
        return Expr::Literal_Expr(false);
    }
